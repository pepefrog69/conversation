...
**BETA** The Searching dialog feature described in this documentation is a beta feature that has been made available for your evaluation. Beta features might be unstable, might change frequently, and might be discontinued with short notice. Beta features also might not provide the same level of performance or compatibility that generally available features provide, and are not intended for use in a production environment.

Use the Search feature to find conditions and titles in your dialog.
...
Folders have no impact on the evaluation order of nodes in the tree from the first to last. As the service travels down the tree, when it encounters a folder, if the folder condition is true, it immediately processes the first node in the folder, and continues down the tree in order from there.
